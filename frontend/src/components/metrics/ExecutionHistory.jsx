  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 rounded-xl border border-blue-100 bg-slate-50 px-3.5 py-2.5">
          ...
          <input
            ...
            className="bg-transparent text-xs text-slate-700 outline-none placeholder:text-slate-500"
          />
        </div>

        <div className="flex items-center gap-2">
          <Filter className="h-4 w-4 text-slate-500" />
          <div className="flex rounded-xl border border-blue-100 bg-slate-50 p-1">
            <button
              type="button"
              onClick={() => setFilter("all")}
              className={`rounded-lg px-3 py-1 text-xs font-medium transition ${
                filter === "all" ? "bg-sky-600 text-white" : "text-slate-600 hover:text-slate-700"
              }`}
            >
              All
            </button>
            <button
              ...
              className={`rounded-lg px-3 py-1 text-xs font-medium transition ${
                filter === "success" ? "bg-emerald-600 text-white" : "text-slate-600 hover:text-slate-700"
              }`}
            >
              Success
            </button>
            <button
              ...
              className={`rounded-lg px-3 py-1 text-xs font-medium transition ${
                filter === "failed" ? "bg-red-600 text-white" : "text-slate-600 hover:text-slate-700"
              }`}
            >
              Failed
            </button>
      ...
      <div className="overflow-x-auto rounded-xl border border-blue-100 bg-white">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-blue-100/60 text-xs">
            {filtered.length === 0 ? (
              <tr>
              ...
              filtered.map((item) => (
                <tr key={item.id} className="transition hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4 text-slate-600">{item.timestamp}</td>
                </tr>
              ))
            )}
          </tbody>
